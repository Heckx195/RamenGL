...
1. Vorberechnung der Punkte auf dem Ring für Top und Bottom, da diese für den Mantel und die Deckel wiederverwendet werden. Anzahl der Ringpunkte wird basierend auf der Anzahl der Slices berechnet.
...
3. Die Normalenvektoren für die Dreiecke sind identisch mit den Positionen der Punkte, da die Normalenvektoren bei einer Kugel von ihrem Mittelpunkt nach außen zeigen. Die UV-Koordinaten werden von (0,0) unten links bis (1,1) oben rechts aufgespannt.
...
2. Möglichkeit: Verwendung von RenderDoc
  - In RenderDoc die gerenderten Frames analysieren. In RenderDoc können die Normalenvektoren im Mesh Viewer unter VS Input (Vertex-Shader) inspiziert werden. Zudem kann überprüft werden, ob diese korrekt an den Fragment-Shader weitergegeben werden (VS Out)
  - "out_Normal = in_Normal;" zu Vertex-Shader hinzugefügt, um die Normalen direkt an den Fragment-Shader weiterzugeben, und in RenderDoc überprüft, ob die Werte korrekt sind.
#figure(
...
- Pitch Down: negative Rotation um die X-Achse
  -> Läuft man ins Positive auf der X-Achse, neigt sie sich nach oben
- Yaw Left: positive Rotation um die Y-Achse
  -> Läuft man ins Positive auf der Y-Achse, dreht man sich nach links
...
- Dies ist besonders bei der Animation des Sonnensystems wichtig, da man für die Planeten zuerst die Translation für den Abstand zur Sonne machen muss und dann erst die Rotation um die Sonne.
