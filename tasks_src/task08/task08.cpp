...
int main(int argc, char** argv)
{
    ...
    pRamen->Init("Task 07 - Shadow-Mapping", 800, 600);
    ...
    {
        fprintf(stderr, "Could not load lightSource shader.\n");
    }

    Shader floorPCFShader{};
    if ( !floorPCFShader.Load("shaders/pcf_floor.vert", "shaders/pcf_floor.frag") )
    {
        fprintf(stderr, "Could not load floor PCF shader.\n");
    }
    ...
    const int SHADOW_MAP_SIZE = 8192; // 1024 // 2048 // 4096
    ...
    // TODO: Aufgabe 7.0) GPU-seitige Schattenberechnung
    glTextureParameteri(textureHandleShadowMap, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE);
    ...
    Vec3f materialEmissive{ 0.0f, 0.0f, 0.0f };
    Vec3f materialAmbient{ 0.1f, 0.1f, 0.1f };
    Vec3f materialDiffuse{ 0.8f, 0.8f, 0.8f };
    Vec3f materialSpecular{ 0.5f, 0.5f, 0.5f };
    ...
    /* Main loop */
    bool isRunning = true;
    SDL_GL_SetSwapInterval(vsync ? 1 : 0); /* 1 = VSync enabled; 0 = VSync disabled */
    ...
    while ( isRunning )
    {
        ...
        // TODO: Aufgabe 7.2) FPS Anzeige
        ImGuiIO& io = ImGui::GetIO();

        // Saves the fps the last 60 seconds.
        static std::deque<float> frameTimes;
        static float frameTimeSum = 0.0f;
        frameTimeSum += io.DeltaTime;
        frameTimes.push_back(io.DeltaTime);
        while (frameTimeSum > 60.0f) {
            frameTimeSum -= frameTimes.front();
            frameTimes.pop_front();
        }
        float avgFPS60s = frameTimes.size() / frameTimeSum;

        ImGui::Begin("PCF controlls");
        ImGui::Text("Application average %.3f ms/frame (%.1f FPS)", 1000.0f / io.Framerate, io.Framerate);
        ImGui::Text("Average FPS (60s):  %.1f", avgFPS60s);
        ImGui::End();

        ImGui::Begin("Blinn-Phong controlls");
        ImGui::ColorPicker3("Emissive", materialEmissive.Data());
        ImGui::ColorPicker3("Ambient", materialAmbient.Data());
        ImGui::ColorPicker3("Diffuse", materialDiffuse.Data());
        ImGui::ColorPicker3("Specular", materialSpecular.Data());
        ImGui::SliderFloat("Shininess", &shininess, 1.0f, 256.0f);
        ImGui::SliderFloat("Reflectivity", &reflectivity, 0.0f, 1.0f);
        ImGui::Checkbox("Aktiviere Fragment-Shading (Phong-Shading)", &usePhongShading);
        ImGui::End();

        ImGui::Begin("Blinn-Phong Lichteigenschaften");
        ImGui::ColorPicker3("Ambient", lightAmbient.Data());
        ImGui::ColorPicker3("Diffuse", lightDiffuse.Data());
        ImGui::ColorPicker3("Specular", lightSpecular.Data());
        ImGui::End();
        ...
        glUniform1f(8, usePCF ? 1 : 0); // Use PCF
        ...
        glUniform1f(8, usePCF ? 1 : 0); // u_UsePCF
        glDrawArrays(GL_TRIANGLES, 0, sphereVertices.size());
