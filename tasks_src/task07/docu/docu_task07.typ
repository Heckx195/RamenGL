...
Damit Sie den Schattentest innerhalb der texture()-GLSL-Funktion durchführen können.
Außerdem müssen Sie anstatt eines sampler2D einen sampler2DShadow im Fragment-Shadercode für Ihre depthmap-Textur verwenden, damit Sie einen vec3 als Texturkoordinaten an texture() übergeben können.
...
== 7.2) GUI - FPS
Fügen Sie in ihr GUI-Fenster eine Textanzeige hinzu, die Ihnen die Frametime und FPS darstellt. Dazu können Sie sich existierender ImGui Funktionalität bedienen.

Testen Sie nun folgendes Szenario bei ausgeschaltetem VSync: Starten Sie Ihr Programm mit unterschiedlichen Shadowmap-Größen und schalten Sie PCF ein/aus und notieren Sie die FPS für:
- 1024x1024
- 2048x2048
- 4096x4096
- 8192x8192
Shadowmap Texturgrößen.
...
- Getestet wurde auf einer RTX2080 (8GB VRAM) mit der Standardfenstergröße von 800x600, mit Blick auf einem Schatten. In den Nvidia-Einstellungen wurden die globalen 3D-Einstellungen für die Applikation deaktiviert, sodass V-Sync sowie eine FPS-Einschränkung von bspw. 165 FPS deaktiviert waren.
- Der nicht signifikante Unterschied der FPS-Werte zwischen den Auflösungen und PCF an/aus ist wahrscheinlich darauf zurückzuführen, dass die Berechnungen noch nicht ins Gewicht gefallen sind, sowie durch mögliche versteckte Optimierungen von Nvidia, die die wirkliche Leistung der Grafikkarte nicht vollständig ausnutzen. Der Unterschied zwischen den Werten ist wahrscheinlich auf verschiedene Systemlasten zurückzuführen, die während der Tests aufgetreten sind.
...
=== Frage 2.:
Wie hat sich die Texturgröße auf die Performance Ihres Programms ausgewirkt? Hat PCF
...
=== Frage 3.:
PCF ermöglicht zwar einen weicheren Übergang am Rand des Schattens, lässt aber (mind.) eine physikalische Begebenheit außer Acht. Welche ist das?
...
== In Bezug auf die vorherige Übung:
...
- Tritt auf, wenn die Resthelligkeit des Halbschattens eines Objekts als Lichtfaktor eines anderen Objekts interpretiert wird, obwohl dieses Objekt eigentlich im Kernschatten liegen sollte.
