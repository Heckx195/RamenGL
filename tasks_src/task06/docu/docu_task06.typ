...
- An diesen Framebuffer wird eine Textur als Depth Attachment gebunden, um die Tiefeninformationen zu speichern.
 ...
 - Besonders wichtig ist die Einstellung der Texture-Parameter das bei einem S/T-Wrap GL_CLAMP_TO_BORDER verwendet wird, damit außerhalb der Shadowmap ein definierter Wert (`borderColor`) zurückgegeben wird, der für den Tiefenvergleich im Hauptpass verwendet wird. Bei der `borderColor` sollte der Rot-Kanal auf 1.0 gesetzt werden, damit außerhalb der Shadowmap/ Licht-Frustum immer true ist und alles beleuchtet wird. Wenn der Rot-Kanal auf 0.0 gesetzt wird, würde außerhalb der Shadowmap alles im Schatten liegen (siehe @fig-border-color).
...
- Für den ersten Durchlauf muss der Frame-Buffer-Object (Shadowmap-Framebuffer) gebunden und die Viewportgröße auf die Größe der Shadowmap gesetzt werden, damit die Tiefenwerte korrekt in die Shadowmap geschrieben werden.
...
- Im zweiten Durchlauf (Hauptpass) wird die Shadowmap als Textur gebunden und im Fragment-Shader gesampelt, um zu bestimmen, ob ein Fragment im Schatten liegt oder nicht.
 ...
 - Fragement-Shader: Die Licht-Sicht-Koordinaten werden zuerst durch `in_LightSpacePos.w` geteilt, um diese in Normalized Device Coordinates (NDC) zu transformieren. Mit dem Bias `*0.5 + 0.5` werden die NDC-Koordinaten in den Texturkoordinatenbereich [0, 1] transformiert.
...
- Die Position der Lichtquelle wird über eine ImGui DragFloat3 gesteuert, die die lightPos-Variable aktualisiert und damit die `lightViewMatrix` beeinflusst und damit die Position der Lichtquell sowie der Shadowmap verändert.

#line(length: 100%)

== 6.6) Renderingartefakte beim Schattenwurf
Sie werden auf einige Renderingartefakte beim Schattenwurf stossen. Dokumentieren Sie, wie man diese nennt, wie sie entstehen und wie diesen entgegengewirkt werden kann.
...
- Hinter der LightCamera liegt nur Schatten
  ...
  - Damit wird ausserhalb der Lichtkegel kein Schatten berechnet.
